...
void Executor::clearViewAccessExprs() {
  klokkos_view_reads.clear();
  klokkos_view_writes.clear();
}

void Executor::addViewAccessExpr(ExecutionState &state, uint64_t view_id, ref<Expr> &expr) {

  if (!state.stack.empty()) {
    StackFrame sf = state.stack.back();
    if (kmodule->isKlokkosWriteToView(sf.caller->inst)) {
      klokkos_view_writes[view_id].insert(expr);
    } else {
      klokkos_view_reads[view_id].insert(expr);
    }
  }
}

void Executor::dumpViewAccessExprs() const {
  dumpViewAccessExprs("*** Reads  ***", klokkos_view_reads);
  dumpViewAccessExprs("*** Writes ***", klokkos_view_writes);
}

void Executor::dumpViewAccessExprs(std::string label, std::map<uint64_t, std::set<klee::ref<klee::Expr>>> map) const {

  if (!map.empty()) {
    outs() << label << ":\n";
    for (auto pr : map) {
      outs() << "View_ID: " << pr.first << '\n';
      for (auto &expr : pr.second) {
        outs() << expr << '\n';
      }
    }
  }
