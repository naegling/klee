...
public:
  ...
  void clearViewAccessExprs();
  void addViewAccessExpr(ExecutionState &state, uint64_t view_id, ref<Expr> &ref);
  void dumpViewAccessExprs() const;
  void dumpViewAccessExprs(std::string label, std::map<uint64_t, std::set<klee::ref<klee::Expr>>> map) const;
  std::map<uint64_t, std::set<klee::ref<klee::Expr>>> klokkos_view_reads;
  std::map<uint64_t, std::set<klee::ref<klee::Expr>>> klokkos_view_writes;
};
